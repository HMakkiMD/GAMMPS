...
from the trajectory of MD "melt" simulation as explained in (DOI). The path to the trajectory and the type of trajectory (e.g., .xtc or .trr) 
...
path_to_dosoutput=path_to_dosinput+'/DOS/'
...
for frame in range(frames):
    ...
    for index, chain in enumerate(chains):
        ...
        new_coordinates=[]
        for image in images:
            for atomnr, atom in enumerate(nopbc):
        ...
        for i in range(len(surronding_mod)):
            ...
            with open(path_to_dosoutput+str((index+1)+frame*len(chains))+'_chain.xyz', 'w') as f:
            ...
            with open(path_to_dosoutput+str((index+1)+frame*len(chains))+'_sur.xyz', 'w') as f:
